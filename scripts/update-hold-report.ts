...
// Update the G4 section to reflect that we have a similarity report
// Find the line with "G4 Originality:" and replace the following lines until the next gate or section.
// We'll do a simple replacement: look for the block starting with "G4 Originality:" and ending before "G5 Editorial:"
...
if (g4Start !== -1) {
  ...
  if (g5Start !== -1) {
    ...
    const newG4Section = `G4 Originality:    HOLD — Classifications: total=50 unique=50 build_now=1 fabricatedSlots=0. BUT:\n                       - Similarity report exists at reports/w0w1-similarity-report.json (preliminary text-shingle based).\n                       - No shared-paragraph %, FAQ duplication, title/meta duplication, or cross-pillar cannibalization scan has been run.\n                       - Similarity report present; need to implement other checks.`;
...
// Also update the "What must happen to leave HOLD" section to reflect that we have produced the similarity report
