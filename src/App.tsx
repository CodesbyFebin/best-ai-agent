...
function ProductProfileView({ slug, navigate }: { slug: string; navigate: (path: string) => void }) {
  ...
  if (!product) {
    return <NotFound title="Agent not found" detail={`No canonical agent entity exists for “${slug}”.`} />;
  }
...
export default function App({ route, navigate }: { route: RouteRecord | null; navigate: (path: string) => void }) {
  ...
  if (pillar) {
    return (
      <>
        <PriorityPillarPage slug={pillar} onNavigate={navigate} />
        <Footer />
      </>
    );
  }
  ...
  if (path === '/') {
    return (
      <>
        <Homepage
          currentView="home"
          onNavigate={(view, siloId, articleSlug, productSlug) => {
          ...
          onOpenRss={() => navigate('/rss.xml')}
          onOpenPseoRepo={() => navigate('/sitemap')}
        />
        <Footer />
      </>
    );
  ...
  if (route?.type === 'pricing') {
    return <><PricingPage onNavigate={navigate} /><Footer /></>;
  }
  ...
  return (
    <>
      <NotFound title="Page not found" detail="This route is not a published canonical page. Use the directory or return to the homepage." />
      <Footer />
    </>
  );
}
