...
import SemanticKnowledgeGraph from './components/SemanticKnowledgeGraph';
...
export default function App() {
  ...
  // Live Sync routing Hash and Pathname parameters for copyable URLs sharing and Search Engine Indexing
  useEffect(() => {
    const handleRouter = () => {
      ...
      if (hash && hash.startsWith('#')) {
        ...
        const author = params.get('author') || params.get('id');
        ...
        if (view) {
          ...
          if (author) setActiveAuthorId(author);
          if (pair) setActiveComparisonPair(pair);
        } else if (product) {
      ...
      // Handle direct pathname routes for Google indexed pages and PSEO architecture
      if (pathname.startsWith('a/best-ai-agent/')) {
        const subPath = pathname.replace('a/best-ai-agent/', '');
        if (subPath.startsWith('reviews/')) {
          const item = subPath.replace('reviews/', '').replace(/\/$/, '');
          routeTo('product', undefined, undefined, item);
        } else if (subPath.startsWith('comparisons/')) {
          const pair = subPath.replace('comparisons/', '').replace(/\/$/, '');
          setActiveComparisonPair(pair);
          setCurrentView('comparison-pair');
        } else if (subPath.startsWith('pricing/') || subPath.startsWith('alternatives/')) {
          const item = subPath.replace(/^(pricing|alternatives)\//, '').replace(/\/$/, '');
          routeTo('product', undefined, undefined, item);
        } else {
          routeTo('article', undefined, subPath);
        }
      } else if (pathname.startsWith('tools/')) {
        const pSlug = pathname.replace('tools/', '');
        routeTo('product', undefined, undefined, pSlug);
      } else if (pathname.startsWith('authors/') || pathname.startsWith('author/')) {
        const authId = pathname.replace(/^authors?\//, '');
        setActiveAuthorId(authId);
        setCurrentView('author');
      } else if (pathname.startsWith('mcp/servers/')) {
        const mcpSlug = pathname.replace('mcp/servers/', '');
        routeTo('article', 'mcp', mcpSlug);
      } else if (pathname === 'compare' || pathname === 'comparison') {
        setCurrentView('compare');
      } else if (['knowledge-graph', 'semantic-graph', 'alsoasked'].includes(pathname)) {
        setCurrentView('knowledge-graph');
      } else if (['pseo-repo', 'pseo-blueprint', 'repo'].includes(pathname)) {
        setIsPseoRepoOpen(true);
      } else if (pathname.includes('-vs-')) {
        setActiveComparisonPair(pathname);
        setCurrentView('comparison-pair');
      } else if (pathname === 'methodology') {
        setCurrentView('methodology');
      } else if (['editorial-policy', 'review-policy', 'corrections-policy'].includes(pathname)) {
        setCurrentView('editorial-policy');
      } else if (['ai-agent-scoring-system', 'scoring-system'].includes(pathname)) {
        setCurrentView('scoring-system');
      } else if (['about', 'team'].includes(pathname)) {
      ...
      } else if (pathname === 'affiliate-disclosure') {
        setCurrentView('disclosure');
      } else if (['glossary', 'glossary-hub'].includes(pathname)) {
        setCurrentView('glossary');
      } else if (['drive', 'google-drive-ai-agent-workspace'].includes(pathname)) {
        setCurrentView('drive');
      } else if (['topical-map', 'ai-agent-market-map', 'ai-agent-roadmap'].includes(pathname)) {
        setCurrentView('topical-map');
      } else if (silos.some(s => s.id === pathname)) {
        routeTo('silo-pillar', pathname);
      } else if (products.some(p => p.slug === pathname)) {
        routeTo('product', undefined, undefined, pathname);
      } else if (pathname.endsWith('-pricing') || pathname.endsWith('-cost-calculator') || pathname.endsWith('-alternatives')) {
        const baseSlug = pathname.replace(/-(pricing|cost-calculator|alternatives)$/, '');
        const matchedProduct = products.find(p => p.slug === baseSlug || p.slug.startsWith(baseSlug));
        if (matchedProduct) {
          routeTo('product', undefined, undefined, matchedProduct.slug);
        } else {
          routeTo('article', undefined, pathname);
        }
      } else {
        routeTo('article', undefined, pathname);
      }
  ...
  useEffect(() => {
    ...
    if (currentView === 'home') {
      schemaData = {
        ...
        "potentialAction": {
          "@type": "SearchAction",
          "target": "https://bestaiagent.in/#view=home&q={search_term_string}",
      ...
      if (s) {
        schemaData = {
          ...
          "url": `https://bestaiagent.in/#view=silo-pillar&silo=${s.id}`,
          "itemListElement": products.map((p, idx) => ({
            "@type": "ListItem",
            "position": idx + 1,
            "name": p.name,
            "url": `https://bestaiagent.in/#view=product&product=${p.slug}`
          }))
        };
      }
    } else if (currentView === 'article') {
          ...
          "@graph": [
            {
              ...
              "itemListElement": [
                ...
                },
                {
                  ...
                  "item": `https://bestaiagent.in/#view=silo-pillar&silo=${p.siloId}`
                },
                {
                  "@type": "ListItem",
                  "position": 3,
                  "name": p.title,
                  "item": `https://bestaiagent.in/#view=article&article=${p.slug}`
                }
              ]
            },
            {
              ...
              "mainEntityOfPage": `https://bestaiagent.in/#view=article&article=${p.slug}`
            }
          ]
        };
      }
          ...
          "@graph": [
            {
              ...
              "@id": `https://bestaiagent.in/#product-${p.id}`,
              "name": p.name,
              "image": p.logoUrl,
    ...
    return (
      <>
        <Homepage
          ...
          onNavigate={(v, silo, slug) => {
            if (v === 'knowledge-graph') {
              setCurrentView('knowledge-graph');
            } else if (v === 'comparison-pair' && slug) {
              setActiveComparisonPair(slug);
              setCurrentView('comparison-pair');
            } else {
              routeTo(v, silo, undefined, slug);
            }
          }}
          onOpenSearch={() => {
            window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-slate-100 flex flex-col font-sans selection:bg-violet-900 selection:text-white development-frame">
      
      {/* DIRECT TRUST DISCLOSURE RIBBON */}
      <div className="bg-slate-950 text-slate-400 text-xs py-2 px-4 border-b border-slate-800 text-center flex flex-wrap items-center justify-center gap-2">
        <span className="inline-block bg-violet-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">India's #1 AI Agent Discovery Platform</span>
        <p className="inline m-0">Transparency First: Independent benchmark evaluations, DPIIT regulations, Hinglish speech checks, and strict DPDP Act compliance audit scores.</p>
        <button onClick={() => routeTo('disclosure')} className="underline text-slate-300 hover:text-white transition font-medium focus:outline-none">Read Affiliate Disclosure</button>
      </div>
      ...
      <header className="sticky top-0 z-50 bg-[#0B0F19]/90 backdrop-blur-md border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => routeTo('home')} className="flex items-center gap-2.5 text-left focus:outline-none rounded p-1">
              <span className="flex items-center justify-center w-9 h-9 bg-gradient-to-tr from-violet-600 via-indigo-600 to-purple-500 text-white rounded-xl shadow-lg shadow-violet-500/25 font-black text-base tracking-tight">AI</span>
              <div>
                <h1 className="text-xl font-extrabold text-white tracking-tight leading-none font-sans">Best<span className="text-violet-400">AIAgent</span>.in</h1>
          ...
          <nav className="hidden lg:flex items-center gap-1 text-xs font-semibold">
            <button onClick={() => routeTo('home')} className={`px-3 py-2 rounded-lg transition ${currentView === 'home' ? 'bg-violet-600/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>AI Agents</button>
            <button onClick={() => routeTo('silo-pillar', 'reviews')} className={`px-3 py-2 rounded-lg transition ${selectedSiloId === 'reviews' && currentView === 'silo-pillar' ? 'bg-violet-600/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>Categories ▾</button>
            <button onClick={() => routeTo('compare')} className={`px-3 py-2 rounded-lg transition ${currentView === 'compare' ? 'bg-violet-600/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>Compare</button>
            <button onClick={() => setCurrentView('knowledge-graph')} className={`px-3 py-2 rounded-lg transition flex items-center gap-1.5 ${currentView === 'knowledge-graph' ? 'bg-cyan-600/20 text-cyan-300 border border-cyan-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>
              <Network className="w-3.5 h-3.5 text-cyan-400" />
              <span>Knowledge Graph</span>
            </button>
            <button onClick={() => routeTo('silo-pillar', 'coding-agents')} className={`px-3 py-2 rounded-lg transition ${selectedSiloId === 'coding-agents' && currentView === 'silo-pillar' ? 'bg-violet-600/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>Built in India 🇮🇳</button>
            <button onClick={() => routeTo('methodology')} className={`px-3 py-2 rounded-lg transition ${currentView === 'methodology' ? 'bg-violet-600/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/60'}`}>Methodology</button>
          </nav>

          {/* Action Tools Buttons */}
          <div className="flex items-center gap-2">
            <button
              onClick={() => setIsPseoRepoOpen(true)}
              className="px-3 py-1.5 rounded-lg bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 text-xs font-bold flex items-center gap-1.5 transition"
              title="Inspect BestAIAgent PSEO Repository Architecture"
            >
              <Code className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">PSEO Repo</span>
            </button>

            <button
              onClick={() => setIsRssModalOpen(true)}
              className="px-3 py-1.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 text-xs font-bold flex items-center gap-1.5 transition"
            >
              <Rss className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">RSS Feed</span>
            </button>
            
            <button onClick={() => routeTo('compare')} className="hidden sm:inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-bold transition shadow-md shadow-violet-600/20">
              <ArrowLeftRight className="w-3.5 h-3.5" />
              <span>Compare ({compareList.length})</span>
            </button>
            <button onClick={() => routeTo('chat')} className="inline-flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold shadow-sm transition">
              <Sparkles className="w-4 h-4" /> Recommend Assistant
            </button>
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 text-slate-600 hover:bg-slate-50 rounded-lg">
      ...
      {/* MOBILE NAV DRAWER */}
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 py-3 px-4 flex flex-col gap-2">
          <p className="text-[10px] text-slate-400 font-bold tracking-wider uppercase px-2">Content Silos</p>
          {silos.map(s => (
            <button key={s.id} onClick={() => routeTo('silo-pillar', s.id)} className="w-full text-left px-3 py-2 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-50 flex items-center justify-between">
              <span>{s.name}</span>
              <ChevronRight className="w-4 h-4 text-slate-400" />
            </button>
          ))}
          <div className="border-t border-slate-100 my-2 pt-2 flex flex-col gap-2">
            <button onClick={() => routeTo('topical-map')} className="w-full text-left px-3 py-2 rounded-lg text-sm font-bold text-emerald-600 hover:bg-slate-50 flex items-center gap-2">
              <Map className="w-4 h-4" /> 100-Pillar Topical SEO Map
            </button>
            <button onClick={() => routeTo('drive')} className="w-full text-left px-3 py-2 rounded-lg text-sm font-bold text-emerald-800 hover:bg-slate-50 flex items-center gap-2">
              <Database className="w-4 h-4" /> Google Drive Workspace Sync
            </button>
            <button onClick={() => routeTo('compare')} className="w-full text-left px-3 py-2 rounded-lg text-sm font-bold text-emerald-700 hover:bg-slate-50 flex items-center gap-2">
              <ArrowLeftRight className="w-4 h-4" /> Comparative Matrix Board
            </button>
            <button onClick={() => routeTo('tuner')} className="w-full text-left px-3 py-2 rounded-lg text-sm font-bold text-indigo-700 hover:bg-slate-50 flex items-center gap-2">
              <Sliders className="w-4 h-4" /> Score Tuning Sliders
            </button>
            <button onClick={() => routeTo('editorial')} className="w-full text-left px-3 py-2 rounded-lg text-sm font-bold text-violet-700 hover:bg-slate-50 flex items-center gap-2">
              <Settings className="w-4 h-4" /> Admin Editorial Workbench
            </button>
          </div>
        </div>
      )}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (item.type === 'product') {
                        routeTo('silo-pillar', 'reviews'); // tool scorecard on pillar page
                      } else {
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {currentView === 'home' && (
          <div className="space-y-12">
            ...
            {/* HOMEPAGE SITEMAP JSON-LD SCHEMAS */}
            <script type="application/ld+json">
              {JSON.stringify({
                "@context": "https://schema.org",
                "@graph": [
                  {
                    ...
                    "url": "https://bestaiagent.in",
                    "name": "BestAIAgent.in",
                    "description": "Compare the best AI agents in India for coding, business automation, WhatsApp, and support.",
                    "publisher": {
                  ...
                  },
                  {
                    ...
                    "name": "BestAIAgent.in",
                    "url": "https://bestaiagent.in",
                    "logo": "https://bestaiagent.in/assets/logo.png",
                    "founders": [
                      {
                        "@type": "Person",
                        "name": "Arshdeep Singh"
                  ...
                  {
                    "@type": "ItemList",
                    "@id": "https://bestaiagent.in/#itemlist",
                    "name": "Best AI Agents in India",
                    "itemListElement": [
                      {
                        "@type": "ListItem",
                        "position": 1,
                        "name": "Cursor AI",
                        "url": "https://bestaiagent.in/#view=product&slug=cursor-ai"
                      },
                      {
                        "@type": "ListItem",
                        "position": 2,
                        "name": "Vapi AI",
                        "url": "https://bestaiagent.in/#view=product&slug=vapi-ai"
                      },
                      {
                        "@type": "ListItem",
                        "position": 3,
                        "name": "CrewAI",
                        "url": "https://bestaiagent.in/#view=product&slug=crewai"
                      }
                    ]
                  }
                ]
            ...
            {/* HERO SECTION */}
            <section className="bg-gradient-to-tr from-slate-900 via-slate-800 to-slate-950 text-white rounded-3xl p-8 md:p-12 shadow-xl relative overflow-hidden">
              <div className="absolute right-0 bottom-0 w-96 h-96 bg-emerald-600/10 rounded-full blur-3xl"></div>
              <div className="relative z-10 max-w-3xl space-y-6">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-emerald-400 text-xs font-semibold">
                  <Star className="w-3.5 h-3.5 fill-emerald-400 text-emerald-400" /> Leading Indian Editorial authority
                </div>
                <h1 className="text-3xl sm:text-5xl font-extrabold tracking-tight leading-tight select-none">
                  Find the Best AI Agents for Business, Coding, & Task Automation
                </h1>
                <p className="text-slate-300 text-base sm:text-lg max-w-2xl font-light">
                  Direct, evidence-based evaluations of AI builders, developer tools, and voice integrations. Complete with localized INR pricing benchmarks and DPDP compliance safety checklists.
                </p>
                <div className="flex flex-wrap gap-3 pt-2">
                  <button onClick={() => routeTo('chat')} className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-sm font-bold rounded-xl transition flex items-center gap-2 shadow-lg shadow-emerald-500/25">
                    <Sparkles className="w-4 h-4 text-current" /> Consult Recommendation Bot
                  </button>
                  <button onClick={() => routeTo('compare')} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white text-sm font-semibold border border-slate-700 rounded-xl transition flex items-center gap-2">
                    <ArrowLeftRight className="w-4 h-4 text-emerald-400" /> Compare Tools
                  </button>
                  <button onClick={() => routeTo('tuner')} className="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-semibold border border-slate-700 rounded-xl transition flex items-center gap-2">
                    <Sliders className="w-4 h-4 text-indigo-400" /> Score Tuner
                  </button>
                </div>
              </div>
            </section>
            ...
            <section className="bg-white border-2 border-emerald-500/30 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
              ...
              <div className="space-y-4">
                <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                  <BookOpen className="w-6 h-6 text-emerald-600" /> What is the best AI agent for Indian businesses in 2026?
                </h2>
                <p className="text-slate-650 text-xs sm:text-sm leading-relaxed font-sans font-medium">
                  The best AI agent depends strictly on your workflow goals. For autonomous coding and development, **Cursor AI** is the highest-rated compiler companion. For conversational voice bots, **Vapi AI** performs best. For regional Indian enterprises requiring secure WhatsApp chat automation, **Yellow.ai** is the premium recommendation. For neutral open-source agent pipeline builds, **CrewAI** and **Flowise** are the premier choices.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 pt-1 text-[11px] sm:text-xs">
                ...
                <div className="p-3 bg-slate-50 border border-slate-150 rounded-xl space-y-1">
                  <p className="text-[10px] text-slate-400 uppercase font-bold font-mono">🛡️ Best for Compliance</p>
            ...
            {/* SIX SILO EXPLORER GRID */}
            <section className="space-y-6">
              <div>
                <h3 className="text-2xl font-bold tracking-tight text-slate-900">Explore Content Silos & Clusters</h3>
                <p className="text-slate-500 text-sm mt-1">We maintain six clear topical categories representing the AI Agent spectrum.</p>
              </div>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {silos.map((s, index) => {
                  let badgeColor = "bg-emerald-50 text-emerald-700 border-emerald-100";
                  if (s.id === 'builders') badgeColor = "bg-blue-50 text-blue-700 border-blue-100";
                  if (s.id === 'coding-agents') badgeColor = "bg-indigo-50 text-indigo-700 border-indigo-100";
                  if (s.id === 'frameworks') badgeColor = "bg-violet-50 text-violet-700 border-violet-100";
                  if (s.id === 'business') badgeColor = "bg-amber-50 text-amber-700 border-amber-100";
                  if (s.id === 'research') badgeColor = "bg-rose-50 text-rose-700 border-rose-100";

                  return (
                    <AnimateOnIntersection key={s.id} delay={index * 0.1}>
                      <div className="bg-white border border-slate-200 hover:border-slate-350 hover:shadow-md transition-all rounded-2xl p-6 flex flex-col justify-between h-full">
                        <div className="space-y-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 border text-xs font-bold rounded-lg ${badgeColor}`}>
                            {s.name}
                          </span>
                          <h4 className="text-lg font-bold text-slate-900">{s.pillarTitle}</h4>
                          <p className="text-slate-500 text-sm line-clamp-3 leading-relaxed">{s.description}</p>
                        </div>
                        <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                          <button onClick={() => routeTo('silo-pillar', s.id)} className="text-xs font-bold text-slate-900 hover:text-emerald-700 transition flex items-center gap-1 uppercase tracking-wider">
                            View Pillar Hub <ArrowRight className="w-3 h-3" />
                          </button>
                          <span className="text-xs font-bold text-slate-400">
                            {siloPages.filter(p => p.siloId === s.id).length} sub-guides
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-6 self-end md:self-center flex-shrink-0 border-t md:border-t-0 md:border-l border-slate-200/80 pt-4 md:pt-0 pl-0 md:pl-6 w-full md:w-auto justify-between md:justify-start">
                      ...
                      <div className="flex items-center gap-2 self-center">
                        <button
                          ...
                          className={`cursor-pointer inline-flex items-center gap-1.5 px-3 py-2 rounded-lg transition text-xs font-bold uppercase tracking-wider ${compareList.includes(p.slug) ? 'bg-violet-600 text-white shadow-md shadow-violet-500/20' : 'bg-slate-800 text-slate-200 hover:bg-slate-700 border border-slate-700'}`}
                        >
                          <ArrowLeftRight className="w-3.5 h-3.5" />
                          <span>{compareList.includes(p.slug) ? 'In Compare' : 'Compare'}</span>
                        </button>
                        
                        <button onClick={() => routeTo('product', undefined, undefined, p.slug)} className="cursor-pointer inline-flex items-center gap-1.5 bg-slate-800 border border-slate-700 hover:bg-slate-700 px-3 py-2 rounded-lg transition text-slate-200 font-bold text-xs uppercase tracking-wider">
                          <span>Profile</span>
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
                <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
                  ...
                  <div className="relative max-w-md w-full">
                    ...
                    <input
                      type="text"
                      className="w-full bg-slate-50 text-slate-850 placeholder-slate-400 pl-9 pr-8 py-2 border border-slate-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      placeholder="Search tools, descriptions, best-fors..."
                      value={dirQuery}
                      onChange={(e) => setDirQuery(e.target.value)}
                    />
                    {dirQuery && (
                      <button onClick={() => setDirQuery('')} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-slate-200">
                  <button
                    ...
                    className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wide transition border ${
                      dirCategory === "All Categories" 
            ...
            <section className="bg-slate-900 text-white rounded-2xl p-6 sm:p-8 relative overflow-hidden flex flex-col md:flex-row items-center gap-8 justify-between">
              ...
              <form onSubmit={submitLeadForm} className="bg-slate-800 border border-slate-700 rounded-xl p-5 w-full max-w-sm space-y-3 shrink-0">
                {leadSuccess ? (
          ...
          <div id="scroll-pillar-pivot" className="space-y-8">
            ...
            <div className="border-b border-slate-200 pb-6">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
                ...
                <button
                  onClick={() => {
                    const shareUrl = `${window.location.origin}${window.location.pathname}#view=silo-pillar&silo=${activeSilo.id}`;
                      ...
                      {totalPagesNeeded > 1 && (
                        <div className="flex items-center justify-between border-t border-slate-100 pt-5 mt-4">
                          ...
                          <div className="text-xs text-slate-500 font-semibold uppercase tracking-wider">
                            ...
                            <span className="ml-3 text-[10px] text-slate-300">({faqsPerPage} items per page)</span>
                          </div>
            ...
            <div className="grid lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-3 space-y-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-4">
                  ...
                  <button
                    onClick={() => {
                      const shareUrl = `${window.location.origin}${window.location.pathname}#view=article&article=${page.slug}`;
                      navigator.clipboard.writeText(shareUrl).then(() => {
  ...
  "@graph": [
    ...
    },
    {
      ...
      "itemListElement": [
        ...
        {/* ==========================================
            VIEW KG: ALSOASKED & SEMANTIC KNOWLEDGE GRAPH
            ========================================== */}
        {currentView === 'knowledge-graph' && (
          <div className="space-y-6">
            <div className="border-b border-slate-800 pb-4">
              <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-2">
                <button onClick={() => routeTo('home')} className="hover:underline text-cyan-400">Home</button>
                <ChevronRight className="w-3 h-3 text-slate-600" />
                <span className="text-slate-300 font-semibold">Semantic Knowledge Graph</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold text-white">AI Agent AlsoAsked & GEO Semantic Entity Graph</h2>
              <p className="text-slate-400 text-sm mt-1">Explore live intent clusters, parent/child queries, entity relationships, and PSEO URL routing for Google Search and Answer Engines.</p>
            </div>
        ...
        {/* ==========================================
            VIEW D: INTERACTIVE COMPARE BOARD
            ========================================== */}
        {currentView === 'compare' && (
        ...
        {currentView === 'methodology' && (
          <MethodologyPage onNavigate={(view, slug) => routeTo(view, undefined, slug)} />
        )}

        {/* EDITORIAL POLICY PAGE */}
        {currentView === 'editorial-policy' && (
          <EditorialPolicyPage onNavigate={(view, slug) => routeTo(view, undefined, slug)} />
        )}

        {/* SCORING SYSTEM PAGE */}
        {currentView === 'scoring-system' && (
          <ScoringSystemPage onNavigate={(view, slug) => routeTo(view, undefined, slug)} />
        )}
        ...
        {currentView === 'author' && (
          <AuthorProfilePage
            ...
            onNavigate={(view, slug) => routeTo(view, undefined, slug)}
          />
        )}

        {/* PROGRAMMATIC COMPARISON PAIR MATRIX PAGE */}
        {currentView === 'comparison-pair' && (
          <ComparisonMatrixPage
            pairSlug={activeComparisonPair}
            onNavigate={(view, slug) => routeTo(view, undefined, slug)}
          />
        )}
      ...
      {/* SITEMAP / FOOTER EXPLORER SYSTEM */}
      <Footer
        onNavigate={(view, siloId, slug) => routeTo(view, siloId, slug)}
        onOpenRss={() => setIsRssModalOpen(true)}
        onOpenPseoRepo={() => setIsPseoRepoOpen(true)}
      />
      ...
      {compareList.length > 0 && currentView !== 'compare' && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 bg-[#111827] border border-violet-500/30 text-white px-5 py-3 rounded-2xl shadow-2xl flex flex-wrap items-center justify-between gap-4 max-w-2xl w-[92%] backdrop-blur-lg">
          <div className="flex items-center gap-2 overflow-x-auto py-1">
            <span className="text-[10px] font-bold text-violet-400 uppercase tracking-widest font-mono">Comparing ({compareList.length}/3):</span>
            {compareList.map(slug => {
              const prod = products.find(p => p.slug === slug);
              return (
                <span key={slug} className="bg-slate-800 text-slate-200 border border-slate-700 text-xs font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1.5 shrink-0">
                  <span>{prod?.name || slug}</span>
                  <button
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="text-xs text-slate-400 hover:text-white transition underline px-2"
            >
              Clear
            </button>
            <button
              onClick={() => routeTo('compare')}
              className="px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-violet-600/30 transition flex items-center gap-1.5"
            >
              <span>View Matrix ({compareList.length})</span>
              <ArrowLeftRight className="w-3.5 h-3.5" />
            </button>
          </div>
        </div>
      ...
      {/* PSEO REPOSITORY ARCHITECTURE VIEWER */}
      <PseoRepoViewer
        isOpen={isPseoRepoOpen}
        onClose={() => setIsPseoRepoOpen(false)}
      />
