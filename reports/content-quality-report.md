# Content Quality Report
Generated: 2026-07-02

- **Total markdown files:** 336
- **Average word count:** 2769
- **Files under 1,500 words:** 127
- **Files with < 10 internal links:** 138
- **Files missing author:** 301

## Longest Pages
- pillars/best-ai-agent-for-crm.md (5039 words, 13 links)
- pricing/github-copilot-pricing.md (5031 words, 14 links)
- pillars/best-ai-agent-for-python.md (4977 words, 15 links)
- tools/cursor-ai.md (4954 words, 14 links)
- pillars/best-ai-agent-for-lead-generation.md (4945 words, 13 links)
- reviews/github-copilot.md (4941 words, 14 links)
- reviews/claude-code.md (4932 words, 15 links)
- reviews/yellow-ai-review.md (4908 words, 14 links)
- pillars/best-ai-agent-for-ecommerce.md (4877 words, 14 links)
- reviews/replit-agent.md (4869 words, 14 links)

## Shortest Pages (may need expansion)
- research/agent-ops.md (817 words, 2 links)
- trust/contact.md (833 words, 3 links)
- authors/priya-iyer.md (839 words, 2 links)
- trust/about.md (846 words, 1 links)
- mcp/servers/docker-server.md (847 words, 1 links)
- mcp/servers/kubernetes-server.md (847 words, 1 links)
- mcp/servers/linkedin-server.md (847 words, 1 links)
- mcp/servers/medium-server.md (847 words, 1 links)
- mcp/servers/playwright-server.md (847 words, 1 links)
- mcp/servers/puppeteer-server.md (847 words, 1 links)

## Recommendations
- Expand pages with < 1500 words to meet minimum threshold
- Add author frontmatter to all pages missing it
- Add internal links to pages with < 10 contextual internal links
- Add `lastUpdated` field to all frontmatter blocks